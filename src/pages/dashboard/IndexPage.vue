      ...
      <div class="q-pa-md q-mb-xl row items-start justify-center q-gutter-sm">
        <q-card class="my-card" v-for="(post, index) in posts" :key="index">
          ...
          <q-card-section class="q-pt-none">
            ...
            <div class="text-caption text-grey">{{ post.description.substring(0, 200) }}...</div>
          </q-card-section>
          <q-separator />
          <q-card-actions>
            <q-btn color="indigo-2" icon-right="favorite" :label="`like ${post.upVoteCount}`" @click="Like(post.id)" />
          </q-card-actions>
        </q-card>
      </div>
    </q-layout>
